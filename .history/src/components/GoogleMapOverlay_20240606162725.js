...
const GoogleMapOverlay = () => {
  ...
  const [imageCenter, setImageCenter] = useState({ x: 0, y: 0 }); // Initialize setImageCenter here
  const mapRef = useRef();
    ...
    if (map) {
      ...
      if (bounds) {
        ...
        img.onload = () => {
          ...
          setImageScale(Math.max(imageScaleWidth, imageScaleHeight));
        };
      }
    }
  }, [imageScale]);
