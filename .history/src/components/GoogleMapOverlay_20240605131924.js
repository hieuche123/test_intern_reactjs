...
const overlayImageStyle = (scale, opacity, imagePosition) => ({
  width: `${100 * scale}%`,
  height: `${100 * scale}%`,
  opacity: opacity,
  position: 'absolute',
...
const GoogleMapOverlay = () => {
  ...
  const [imagePosition, setImagePosition] = useState({ top: '50%', left: '50%' }); // Thêm state mới
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      <GoogleMap
        ...
        onBoundsChanged={() => {
          ...
          if (map) {
            ...
            const imageCenterX = (newCenter.lng() - center.lng) / (bounds.east - bounds.west) * mapSize.width;
            const imageCenterY = (center.lat - newCenter.lat()) / (bounds.north - bounds.south) * mapSize.height;
            // Cập nhật vị trí mới của ảnh
            setImagePosition({
              top: `calc(50% - ${imageCenterY*6}px)`,
              left: `calc(50% - ${imageCenterX*mapSize}px)`,
            });
          }
        }}
      />
      <img
        src={anh}
        alt="Overlay"
        style={overlayImageStyle(imageZoom, opacity, imagePosition)}
      />
