...
const GoogleMapOverlay = () => {
  ...
  const calculateImagePosition = () => {
    ...
    if (map) {
      ...
      const overlayLatLng = new window.google.maps.LatLng(mapCenter.lat, mapCenter.lng);
      const overlayPoint = overlayProjection.fromLatLngToPoint(overlayLatLng);
      const worldCoordinateCenter = new window.google.maps.Point(overlayPoint.x, overlayPoint.y);
      const pixelCoordinateCenter = new window.google.maps.Point(
        worldCoordinateCenter.x * Math.pow(2, mapZoom),
        worldCoordinateCenter.y * Math.pow(2, mapZoom)
      );
      return {
        top: pixelCoordinateCenter.y,
        left: pixelCoordinateCenter.x,
      };
    }
    return { top: 0, left: 0 };
  };
