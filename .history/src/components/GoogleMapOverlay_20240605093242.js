...
const GoogleMapOverlay = () => {
  ...
  const onMapLoad = useCallback((map) => {
    ...
    map.addListener('center_changed', () => {
      ...
      const latLngToPoint = (latLng) => {
        const scale = Math.pow(2, map.getZoom());
        const worldCoordinate = new window.google.maps.Point(
          (latLng.lng() + 180) / 360 * scale * 256,
          (latLng.lat() + 90) / 180 * scale * 256
        );
        return worldCoordinate;
      };

      const mapCenterPoint = latLngToPoint(mapCenter);
      const mapContainerDiv = mapRef.current.getDiv();
      const mapContainerRect = mapContainerDiv.getBoundingClientRect();

      const top = mapContainerRect.top + mapContainerRect.height / 2;
      const left = mapContainerRect.left + mapContainerRect.width / 2;
