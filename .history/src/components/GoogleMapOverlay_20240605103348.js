...
const overlayImageStyle = (scale, opacity, mapCenter, imageCenter) => ({
  width: `${100 * scale}%`,
  height: `${100 * scale}%`,
  opacity: opacity,
  position: 'absolute',
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      <GoogleMap
        ...
        onBoundsChanged={() => {
          ...
          if (map) {
            ...
            // Tính toán vị trí trung tâm của hình ảnh
            const mapSize = map.getDiv().getBoundingClientRect();
            const imageCenterX = (newBounds.getNorthEast().lng() - center.lng) / (newBounds.getNorthEast().lng() - newBounds.getSouthWest().lng()) * mapSize.width;
            const imageCenterY = (newBounds.getNorthEast().lat() - center.lat) / (newBounds.getNorthEast().lat() - newBounds.getSouthWest().lat()) * mapSize.height;
      ...
      <img
        src={anh}
        alt="Overlay"
        style={overlayImageStyle(imageZoom, opacity, center, imageCenter)}
      />
