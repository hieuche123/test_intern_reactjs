...
import { GoogleMap, useLoadScript, OverlayView, useMap } from '@react-google-maps/api';
  ...
  const MapZoomListener = () => {
    ...
    useEffect(() => {
      const listener = map.addListener('zoom_changed', () => {
        // Update image size based on map zoom level
        if (imageSize.width && imageSize.height) {
          const newScale = map.getZoom() / 14; // Adjust this ratio according to your preference
          setScale(newScale);
        }
      });
      return () => {
