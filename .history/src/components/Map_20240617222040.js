...
function App() {
  ...
  const handleInputChange = (event) => {
    ...
    if (section === 'center') {
      ...
      // Kiểm tra giá trị nhập vào có phải là số thực không
      const floatValue = parseFloat(value);
      if (!isNaN(floatValue)) {
        setSizeInputs({ ...sizeInputs, [coord]: floatValue });
      }
    } else {
      // Kiểm tra giá trị nhập vào có phải là số thực không
      const floatValue = parseFloat(value);
      if (!isNaN(floatValue)) {
        setCornerInputs({ 
          ...cornerInputs, 
          [section]: { ...cornerInputs[section], [coord]: floatValue } 
        });
      }
    }
  };
  ...
  const handleFormSubmit = (event) => {
    ...
    setImageSize({ width: sizeInputs.width, height: sizeInputs.height });
    setCurrentSize({
  ...
  return (
    <div>
      <div {...getRootProps()} style={{ position: 'absolute', top: 0, left: 10, zIndex: 1, padding: 10, background: 'white', borderRadius: 4 }}>
        <input {...getInputProps()} />
        <p>Kéo & thả ảnh hoặc click để chọn ảnh</p>
      </div>
      <div style={{ position: 'absolute', top: 55, left: 10, zIndex: 1,background: 'white', padding: 10, borderRadius: 4 }}>
        <div>
        ...
        <div>
          ...
          <Slider min={0} max={1} step={0.0001} value={scale} onChange={setScale} />
        </div>
      </div>
      <GoogleMap
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 10, zIndex: 1, background: 'white', padding: 10, borderRadius: 4 }}>
