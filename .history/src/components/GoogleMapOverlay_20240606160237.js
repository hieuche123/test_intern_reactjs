...
const GoogleMapOverlay = () => {
  ...
  const updateImagePosition = useCallback(() => {
    ...
    if (map) {
      ...
      if (bounds) {
        ...
        const imageCenterX = (center.lng - sw.lng()) / (ne.lng() - sw.lng()) * mapDiv.width;
        const imageCenterY = (ne.lat() - center.lat) / (ne.lat() - sw.lat()) * mapDiv.height;
  ...
  const onMapLoad = useCallback((map) => {
    ...
    // Update image position once after the map is loaded
    updateImagePosition();
  }, [updateImagePosition]);
  ...
  }, [isLoaded, mapZoom, updateImagePosition]);
