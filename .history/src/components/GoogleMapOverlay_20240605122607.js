...
const GoogleMapOverlay = () => {
  ...
  const updateImagePosition = () => {
    ...
    if (map) {
      ...
      const imageCenterX = (newCenter.lng() - center.lng) / (Math.pow(2, 13 - map.getZoom())) * mapSize.width;
      const imageCenterY = (center.lat - newCenter.lat()) / (Math.pow(2, 13 - map.getZoom())) * mapSize.height;
