...
const GoogleMapOverlay = () => {
  ...
  const [centerPosition, setCenterPosition] = useState(center); // Thêm state mới
  ...
  const handleMouseMove = (e) => {
    ...
    if (map) {
      ...
      const latLng = map.getProjection().fromContainerPixelToLatLng(new window.google.maps.Point(e.clientX, e.clientY));
      if (bounds && bounds.contains(latLng)) {
        const newCenter = { lat: latLng.lat(), lng: latLng.lng() };
        setCenterPosition(newCenter);
        const imageCenterX = (newCenter.lng - center.lng) / (bounds.toSpan().lng()) * mapSize.width;
        const imageCenterY = (center.lat - newCenter.lat) / (bounds.toSpan().lat()) * mapSize.height;
  ...
  return (
    <div
      ...
      onMouseMove={handleMouseMove} // Sử dụng sự kiện onMouseMove
    >
      <GoogleMap
        mapContainerStyle={mapContainerStyle}
        center={centerPosition} // Sử dụng vị trí trung tâm mới
        zoom={mapZoom}
        options={mapOptions}
        onLoad={onMapLoad}
      />
