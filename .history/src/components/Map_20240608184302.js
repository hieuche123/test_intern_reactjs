...
function App() {
  ...
  const handleDragStart = (event) => {
    ...
    imgRef.current.dragOffsetX = event.clientX - point.x;
    imgRef.current.dragOffsetY = event.clientY - point.y;
  };
  ...
  const handleDrag = (event) => {
    ...
    const newPoint = new window.google.maps.Point(
      event.clientX - imgRef.current.dragOffsetX,
      event.clientY - imgRef.current.dragOffsetY
    );
    const newLatLng = overlayProjection.fromDivPixelToLatLng(newPoint);
        ...
        {image && (
          ...
          >
            <div
              ...
              onMouseUp={() => imgRef.current.dragging = false}
              onMouseLeave={() => imgRef.current.dragging = false}
            >
              <img
                src={image}
                alt="Overlay"
