...
const overlayImageStyle = (scale, opacity, imageCenter, mapSize) => ({
  width: `${100 * scale}%`,
  height: `${100 * scale}%`,
  opacity: opacity,
  position: 'absolute',
  top: `calc(50% - ${imageCenter.y / (mapSize.height / 2) * scale}px)`,
  left: `calc(50% - ${imageCenter.x / (mapSize.width / 2) * scale}px)`,
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      <img
        src={anh}
        alt="Overlay"
        style={(mapSize) => overlayImageStyle(imageZoom, opacity, imageCenter, mapSize)}
      />
