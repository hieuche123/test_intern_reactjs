  ...
  useEffect(() => {
    if (image) {
      ...
      img.onload = () => {
        ...
        const north = mapCenter.lat + (img.height / 5) * 0.00009;
        const south = mapCenter.lat - (img.height / 5) * 0.00009;
        const east = mapCenter.lng + (img.width / 5) * 0.00009;
        const west = mapCenter.lng - (img.width / 5) * 0.00009;
        ...
        >
          {overlayBounds && (
            <>
              ...
              />
              <div style={{ position: 'absolute', top: '10px', right: '10px', zIndex: '1001', backgroundColor: 'white', padding: '10px', borderRadius: '5px' }}>
                <p>Image Size: {imageSize.width} x {imageSize.height}</p>
                <p>Image Center: {mapCenter.lat}, {mapCenter.lng}</p>
              </div>
            </>
          )}
        </GoogleMap>
