...
const overlayImageStyle = (scale, opacity, left, top) => ({
  width: `${100 * scale}%`,
  height: `${100 * scale}%`,
  opacity: opacity,
  position: 'absolute',
...
const GoogleMapOverlay = () => {
  ...
  const mapBounds = mapRef.current ? mapRef.current.getBounds() : null;
  const mapCenterLatLng = mapBounds ? mapBounds.getCenter() : center;
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      <img
        src={anh}
        alt="Overlay"
        style={overlayImageStyle(imageZoom, opacity, `${mapCenterLatLng.lng()}`, `${mapCenterLatLng.lat()}`)}
      />
