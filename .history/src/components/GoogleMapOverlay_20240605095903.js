...
const overlayImageStyle = (scale, opacity, imageCenter, mapCenter) => {
  const offsetX = (mapCenter.lng - imageCenter.lng) * (100 / scale);
  const offsetY = (imageCenter.lat - mapCenter.lat) * (100 / scale);
  return {
    width: `${100 * scale}%`,
    height: `${100 * scale}%`,
    opacity: opacity,
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      <img
        src={anh}
        alt="Overlay"
        style={overlayImageStyle(imageZoom, opacity, imageCenter, mapCenter)}
      />
