...
const GoogleMapOverlay = () => {
  ...
  const [previousCenter, setPreviousCenter] = useState(center); // Lưu vị trí trung tâm trước đó
  const [previousZoom, setPreviousZoom] = useState(13); // Lưu tỷ lệ zoom trước đó
  const mapRef = useRef();
  ...
  const onMapLoad = useCallback((map) => {
    ...
    map.addListener('zoom_changed', () => {
      ...
      setMapZoom(newZoom);
      setImageZoom(prevZoom => Math.pow(2, newZoom - 13)); // Tính tỷ lệ zoom của ảnh dựa trên zoom level của Google Map
    });
  }, []);
  ...
  useEffect(() => {
    ...
    if (map) {
      const previousZoomLevel = map.getZoom();
      const previousMapCenter = map.getCenter();
      setPreviousZoom(previousZoomLevel);
      setPreviousCenter(previousMapCenter);
    }
  }, []);
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      <GoogleMap
        ...
        center={previousCenter} // Sử dụng vị trí trung tâm trước đó
        zoom={mapZoom}
      ...
      <img
        src={anh}
        alt="Overlay"
        style={overlayImageStyle(imageZoom, opacity)}
      />
