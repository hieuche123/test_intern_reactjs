...
const overlayImageStyle = (scale, opacity, imageCenter) => ({
  ...
  opacity: opacity,
  position: 'absolute',
  top: `calc(50% + ${imageCenter.y * scale}px)`,
  left: `calc(50% + ${imageCenter.x * scale}px)`,
      ...
      <GoogleMap
        ...
        onBoundsChanged={() => {
          ...
          if (map) {
            ...
            // Tính toán sự thay đổi theo tỷ lệ bản đồ
            const latDiff = center.lat - newCenter.lat();
            const lngDiff = newCenter.lng() - center.lng;
        
            // Chuyển đổi sự thay đổi này thành pixel
            const imageCenterX = (lngDiff / (bounds.east - bounds.west)) * mapSize.width;
            const imageCenterY = (latDiff / (bounds.north - bounds.south)) * mapSize.height;
