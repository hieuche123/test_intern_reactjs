...
const GoogleMapOverlay = () => {
  ...
  const onMapMouseMove = (e) => {
    ...
    if (map) {
      ...
      const mapCenter = map.getProjection().fromContainerPixelToLatLng(new window.google.maps.Point(mouseX, mouseY));
      map.setCenter(mapCenter);
    }
  };

  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }} onMouseMove={onMapMouseMove}>
      <GoogleMap
        ...
        onBoundsChanged={() => {
          ...
          if (map) {
            ...
            const mapSize = map.getDiv().getBoundingClientRect();
            const imageCenterX = (newCenter.lng() - center.lng) / (mapSize.width / 2) * (mapSize.width / 2) * imageZoom;
            const imageCenterY = (center.lat - newCenter.lat()) / (mapSize.height / 2) * (mapSize.height / 2) * imageZoom;
