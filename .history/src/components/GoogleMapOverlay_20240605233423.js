...
const overlayImageStyle = (scale, opacity, imageCenter) => ({
  ...
  opacity: opacity,
  position: 'absolute',
  top: `calc(50% - ${imageCenter.y / (window.innerHeight / 2) * scale}px)`,
  left: `calc(50% - ${imageCenter.x / (window.innerWidth / 2) * scale}px)`,
      ...
      <GoogleMap
        ...
        onBoundsChanged={() => {
          ...
          if (map) {
            ...
            const imageCenterX = (newCenter.lng() - center.lng) / (window.innerWidth / 2);
            const imageCenterY = (center.lat - newCenter.lat()) / (window.innerHeight / 2);
