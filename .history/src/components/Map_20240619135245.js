...
function App() {
  const { isLoaded, loadError } = useLoadScript({
    googleMapsApiKey: 'AIzaSyA3bsDl1xddiU_w38hA-fsGea8kWsp5uJM', // Thay YOUR_API_KEY bằng API key của bạn
    libraries,
  });

  const [image, setImage] = useState(null);
  const [imageObjectURL, setImageObjectURL] = useState(null); // Sử dụng URL của ảnh thay vì base64
  ...
  const onDrop = acceptedFiles => {
    ...
    const fileSizeLimit = 10000 * 1024 * 1024; // Giới hạn kích thước ảnh là 10MB
    if (file.size > fileSizeLimit) {
      alert('File ảnh quá lớn, vui lòng chọn ảnh có kích thước nhỏ hơn 10MB.');
      return;
    }
    ...
    reader.onloadend = () => {
      const img = new Image();
      img.onload = () => {
        setImageObjectURL(URL.createObjectURL(file)); // Sử dụng URL tạm thời thay vì base64
        setImage(reader.result);
        const newWidth = img.width;
        const newHeight = img.height;
        setImageSize({ width: newWidth, height: newHeight });
        setCurrentSize({ width: newWidth * scale, height: newHeight * scale });
        setSizeInputs({ width: newWidth, height: newHeight });
      };
      img.src = URL.createObjectURL(file); // Load ảnh để lấy kích thước
    };
