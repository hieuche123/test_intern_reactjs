...
const overlayImageStyle = (zoom, opacity) => ({
  width: `${100 / (zoom * 2)}%`, // Adjusted scale based on zoom level
  height: `${100 / (zoom * 2)}%`, // Adjusted scale based on zoom level
  opacity: opacity,
  position: 'absolute',
  top: '50%',
  left: '50%',
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      <img
        src={anh}
        alt="Overlay"
        style={overlayImageStyle(zoom, opacity)} // Pass zoom level to the style function
      />
