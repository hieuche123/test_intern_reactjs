    ...
    <div style={{ position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }}>
      <GoogleMap
        ...
        onBoundsChanged={() => {
          ...
          if (map) {
            const newCenter = map.getCenter(); // Lấy vị trí trung tâm mới của Google Map
            const mapSize = map.getDiv().getBoundingClientRect(); // Lấy kích thước của Google Map
            const imageCenterX = (newCenter.lng() - bounds.west) / (bounds.east - bounds.west) * mapSize.width;
            const imageCenterY = (bounds.north - newCenter.lat()) / (bounds.north - bounds.south) * mapSize.height;
